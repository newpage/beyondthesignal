...
public final class ReplicationMessageSerializer {
    ...
    public JsonObject serialize(SessionEvent event) {
        ...
        if (event.type() == SessionEventType.SHIP_STATE_UPDATED) {
            ...
            return envelope(event.type().name(), event.sessionId().toString())
                ...
                .put("shieldStrength", payload.getOrDefault("shieldStrength", 100))
                .put("hullIntegrity", payload.getOrDefault("hullIntegrity", 100))
                .put("enemyWeaponCooldownTicks", payload.getOrDefault("enemyWeaponCooldownTicks", 0))
                .put("torpedoesRemaining", payload.getOrDefault("torpedoesRemaining", 0))
                .put("enemyShotsFired", payload.getOrDefault("enemyShotsFired", 0))
                .put("destroyed", payload.getOrDefault("destroyed", false))
    ...
    public JsonObject serializeSnapshot(ShipState state) {
        ...
        return envelope("SHIP_STATE_SNAPSHOT", state.sessionId().toString())
            ...
            .put("shieldStrength", state.shieldStrength())
            .put("hullIntegrity", state.hullIntegrity())
            .put("enemyWeaponCooldownTicks", state.enemyWeaponCooldownTicks())
            .put("torpedoesRemaining", state.torpedoesRemaining())
            .put("enemyShotsFired", state.enemyShotsFired())
            .put("destroyed", state.destroyed())
